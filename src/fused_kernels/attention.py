...
def _decode_kernel(
    ...
    # Load 1D Q query (since seq_len_q == 1)
    q_ptrs = Q + (batch_idx * stride_qb + head_idx * stride_qh + 0 * stride_qm + offs_d * stride_qd)
    ...
    for start_n in range(0, seq_len_kv, BLOCK_N):
        ...
        k = tl.load(k_ptrs, mask=((start_n + offs_n[:, None]) < seq_len_kv) & (offs_d[None, :] < head_dim), other=0.0).to(tl.float32)
        v = tl.load(v_ptrs, mask=((start_n + offs_n[:, None]) < seq_len_kv) & (offs_d[None, :] < head_dim), other=0.0).to(tl.float32)

        # 1D Vector-Matrix multiplication (GEMV)
        qk = tl.sum(q[None, :] * k, axis=1) * sm_scale
        qk = tl.where(start_n + offs_n < seq_len_kv, qk, float("-inf"))
    ...
    out_ptrs = Out + (batch_idx * stride_ob + head_idx * stride_oh + 0 * stride_om + offs_d * stride_od)
